...
//   title: 'Request a Ryd', // Changed Ride to Ryd
// };

const rydRequestFormSchema = z.object({ // Changed rideRequestFormSchema to rydRequestFormSchema
  ...
  date: z.date({ required_error: "Date of ryd is required." }), // Changed ride to ryd
  time: z.string().regex(/^([01]\d|2[0-3]):([0-5]\d)$/, "Invalid time format (HH:MM)."),
  notes: z.string().optional(),
});

type RydRequestFormValues = z.infer<typeof rydRequestFormSchema>; // Changed RideRequestFormValues
...
export default function RydRequestPage() { // Changed RideRequestPage
  const { toast } = useToast();
  const [isSubmitting, setIsSubmitting] = React.useState(false);

  const form = useForm<RydRequestFormValues>({ // Changed RideRequestFormValues
    resolver: zodResolver(rydRequestFormSchema), // Changed rideRequestFormSchema
    defaultValues: {
      time: "09:00", // Default time
    }
  });

  function onSubmit(data: RydRequestFormValues) { // Changed RideRequestFormValues
    setIsSubmitting(true);
    console.log("Ryd Request Data:", data); // Changed Ride Request Data
    // Simulate API call
    setTimeout(() => {
      toast({
        title: "Ryd Requested!", // Changed Ride to Ryd
        description: `Your request for a ryd to ${data.eventId ? mockEvents.find(e=>e.id===data.eventId)?.name : data.eventName || data.destination} has been submitted.`, // Changed ride to ryd
      });
  ...
  return (
    <>
      <PageHeader
        title="Request a Ryd" // Changed Ride to Ryd
        description="Fill out the details below to request a ryd to an event or destination." // Changed ride to ryd
      />
      <Card className="w-full max-w-2xl mx-auto shadow-xl">
        <CardHeader>
          <CardTitle className="font-headline text-xl">Ryd Details</CardTitle> {/* Changed Ride to Ryd */}
          <CardDescription>Please provide accurate information for your ryd request.</CardDescription> {/* Changed ride to ryd */}
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col">
                      <FormLabel>Date of Ryd</FormLabel> {/* Changed Ride to Ryd */}
                      <Popover>
              ...
              <Button type="submit" className="w-full" disabled={isSubmitting}>
                {isSubmitting ? (
                  ...
                  <><Car className="mr-2 h-4 w-4" /> Submit Ryd Request</> // Changed Ride to Ryd
                )}
              </Button>
            </form>
          </Form>
